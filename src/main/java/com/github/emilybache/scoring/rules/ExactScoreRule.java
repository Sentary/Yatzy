...
/**
 * @ExactScoreRule Represent a the exact rule that is configured by the desired
 *                 exact dice score parameter (exactDiceStatus) This rule handle
 *                 dynamically the rules (ones, twos, threes, fours, fives and
 *                 sixes)
 * @author Ismael
 */
public class ExactScoreRule implements ScoringRule {
	...
	// The constructor has the parameter exactDiceStatus that represent the given
	// exact dice score for comparison
	...
	public int getRuleScore(final EnumMap<DiceStatus, Integer> diceCountByStatus) {
		...
		// Return the default rule score (0) if the desired exact score in null or there
		// is no scored dice matching the exact score
